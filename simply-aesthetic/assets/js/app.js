(() => {
    ...
    function initSliders() {
        if (document.querySelector(".treatments__slider")) new core(".treatments__slider", {
        ...
        if (document.querySelector(".single-tmp__slider")) new core(".single-tmp__slider", {
            modules: [Navigation],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
            ...
            loop: true,
            navigation: {
                prevEl: ".single-tmp_swiper-button-prev",
                nextEl: ".single-tmp_swiper-button-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1,
	...
	function swiperCard() {
		  if (window.innerWidth <= 800) {
			if (!init) {
			  init = true;
			  swiper = new core(".reviews__slider", {
				modules: [Navigation],
				slidesPerView: 1.2,
				spaceBetween: 15,
				autoHeight: true,
 ...
 */
    ...
    function documentActions(e) {
        ...
        if (targetElement.parentElement.classList.contains("has-children") && targetElement.parentElement.classList.contains("menu-item-depth-0")) {
            e.preventDefault();
	...
	*/
    ...
    window.onload = () => {
        const subMenuWrap = document.querySelectorAll('.submenu__wrap');
        const subMenuTitle = document.querySelectorAll('.submenu__title');
        const subMenuTitleEl = document.querySelectorAll('.sub-menu-title');

        function fixDesktopMenu() {
            const menuDepth2 = document.querySelectorAll('.menu-depth-2');

            menuDepth2.forEach((item) => {
                const itemHeight = item.offsetHeight;

                item.closest('.submenu__wrap').style.minHeight = itemHeight + "px";

                console.log(item.closest('.submenu__wrap'));
            });
        }
        ...
        function countSubMenu() {
            const countSubMenu = document.querySelectorAll('.menu-depth-1');

            countSubMenu.forEach((e) => {
                const countSubMenuItems = e.querySelectorAll('.menu-item-depth-1');

                if (window.outerWidth >= 991) {
                    if (countSubMenuItems.length > 6) {
                        const ItemFr = Math.floor(countSubMenuItems.length / 2);
                        let styleFrs = "";
                        for (let i = 0; i < ItemFr; i++) {
                            styleFrs += " 1fr";
                        }
                        e.style.gridTemplateRows = styleFrs;
                    }
                }
            });
        ...
        subMenuWrap.forEach((e) => {
            const titleReplase = e.querySelector('.submenu__title');
            const titleRemove = e.querySelector('.sub-menu-title');
            const titleReplaseHtml = e.querySelector('.sub-menu-title a');

            titleRemove.style.display = "none";
            titleReplase.innerHTML = titleReplaseHtml.innerHTML + "<span class='mobile-return'>Return</span>";
        });
    }
